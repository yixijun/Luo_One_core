...
type Email struct {
	...
	AccountID      uint      `gorm:"not null;uniqueIndex:idx_account_message_id;index:idx_account_date" json:"account_id"`
	MessageID      string    `gorm:"size:255;not null;uniqueIndex:idx_account_message_id" json:"message_id"`
	Subject        string    `gorm:"size:500" json:"subject"`
	FromAddr       string    `gorm:"size:255;index" json:"from"`
	ToAddrs        string    `gorm:"type:text" json:"to"` // JSON array stored as string
	Date           time.Time `gorm:"index:idx_account_date" json:"date"`
